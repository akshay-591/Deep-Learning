    ...
    def __init__(self, X, weights, HiddenActivation, OutputActivation):
        ...
        for i in range(len(weights) + 1):
            ...
            if i == 0:  # for initial layer
                ...
                # add bias
                po = self.Activated_Outputs['Hidden_outputs' + str(i - 1)]  # previous output
...
def FFProp(X, weights, Activation):
    """
    This Method is used to Execute FeedForward Propagation algorithm.
    ...
    for i in range(len(weights) + 1):
        ...
        if i == 0:  # for initial layer
            ...
            X = mat.c_[mat.ones(X.shape[0]), X]
            # Calculate output of Hidden layer0
            prediction = HypothesisFunc.predict(X, weights['weights' + str(i)], Activation)
            ...
            prediction = HypothesisFunc.predict(po, weights['weights' + str(i)], Activation)
            Layer_outputs.update(
                {"Hidden_outputs" + str(i): mat.c_[mat.ones(prediction.shape[0]), prediction]})
            ...
            po = Layer_outputs['Hidden_outputs' + str(i - 1)]  # previous output
            prediction = HypothesisFunc.predict(po, weights['weights' + str(i)], Activation="Sigmoid")
            Layer_outputs.update(
