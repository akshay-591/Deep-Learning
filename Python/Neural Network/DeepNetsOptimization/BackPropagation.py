...
from DeepNetsOptimization.ForwardPropagation import FFProp, Feed
...
def BackProp(param, X, Y, inputUnits, outputUnits, numHiddenLayer, numHiddenUnits, HiddenActivation, OutputActivation,
    ...
    #Layer_outputs = FFProp(X, weights, Activation)
    ...
    output = FFModel.Activated_Outputs['output_layer']
    ...
    for i in reversed(range(len(weights))):
        ...
        if i == numHiddenLayer:
            ...
            delta_output = mat.dot(small_delta_output.transpose(), FFModel.Activated_Outputs['Hidden_outputs' + str(i - 1)])
        ...
        if 0 < i < numHiddenLayer:
            ...
            if HiddenActivation == "Sigmoid":
                out = FFModel.Activated_Outputs['Hidden_outputs' + str(i)]
            derivatives, numTrace = CommonCodeBackProp(i=i,
                                                       OutPut=out,
                                                       Input=FFModel.Activated_Outputs['Hidden_outputs' + str(i - 1)],
